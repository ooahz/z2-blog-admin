...
class PureHttp {
  ...
  public formatData(response: any, rawData: boolean = false) {
    try {
      if (!response) {
        message("未获取到有效数据", {type: "error"});
        return "";
      }
      const {state, data, code} = unref(response);
      if (state === "success") {
  ...
  public async request<T>(
    ...
    const response: any = await this.requestData(method, url, param, axiosConfig);
    return this.formatData(response, true);
  }

  public async requestAndFormatData<T>(
    method: RequestMethods,
    url: string,
    param?: AxiosRequestConfig,
    axiosConfig?: PureHttpRequestConfig
